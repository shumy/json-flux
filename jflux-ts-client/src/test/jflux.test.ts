...
describe("JFlux Test", () => {
  ...
  it("pub/sub", (done) => {
    let client = JFlux.client('test-(pub/sub)')
  ...
  it("request/reply", (done) => {
    let client = JFlux.client('test-(request/reply)')
  ...
  it("request/stream", (done) => {
    let client = JFlux.client('test-(request/stream)')
    ...
    client.stream('srv:' + srvName + ':multipleHello', ['Micael', 'Alex', 'Pedro']).subscribe(data => {
        ...
        if (n == 3) {
          expect(data).to.equal('multipleHello Pedro')
          done()
        }
      })
  })
  ...
  it("request/stream-cancel", (done) => {
    let client = JFlux.client('test-(request/stream-cancel)')
